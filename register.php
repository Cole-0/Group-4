...
if (isset($_POST['submit'])) {
    ...
    if (empty($fname) || empty($lname) || empty($position) || empty($contactno) || empty($course) || empty($email) || empty($_POST['password'])) {
        ...
        $position = filter_var(strip_tags($position), FILTER_SANITIZE_STRING); 
        ...
        if (strlen($password) < 6 || strlen($password) > 15 || !preg_match('/[A-Z]/', $password) || !preg_match('/[a-z]/', $password) || !preg_match('/[0-9]/', $password)) {
            ...
            if (count($resultEmail) > 0) {
                ...
                // The email is available, proceed with user registration
                // Insert the user data into the database
                $hashedPassword = password_hash($password, PASSWORD_DEFAULT);

                $stmtInsert = $conn->prepare("INSERT INTO tbl_users (fname, lname, position, contactno, course, password, email, activation_code) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                $stmtInsert->bindParam(1, $fname);
                $stmtInsert->bindParam(2, $lname);
                $stmtInsert->bindParam(3, $position);
                $stmtInsert->bindParam(4, $contactno);
                $stmtInsert->bindParam(5, $course);
                $stmtInsert->bindParam(6, $hashedPassword);
                $stmtInsert->bindParam(7, $email);
                $stmtInsert->bindParam(8, $activation_code);
                        ...
                        <form method="post" action="register.php">
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-control custom-border" name="position">
                            ...
                            <div class="col-12 mb-2">
                                ...
                                <div class="input-group mb-2">
                                    <input type="password" class="form-control custom-border" name="confirmPassword" id="confirm-password" placeholder="Confirm your password" aria-label="Confirm Password" aria-describedby="password-addon-2">
